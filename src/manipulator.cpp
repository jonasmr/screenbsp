...
bool ManipulatorTranslate::DragBegin(v2 vDragStart, v2 vDragEnd, SObject* pSelection)
{
	...
	v3 vDir = DirectionFromScreen(vDragStart, g_WorldState.Camera);
	float fIntersect = rayboxintersect(g_WorldState.Camera.vPosition, vDir, pSelection->mObjectToWorld, pSelection->vSize);
	vBasePoint = g_WorldState.Camera.vPosition + vDir * fIntersect;
...
void ManipulatorTranslate::DragUpdate(v2 vDragStart, v2 vDragEnd)
{
	...
	v3 vNewPoint = g_WorldState.Camera.vPosition + vDirEnd * fBaseDist;
...
void ManipulatorRotate::DragUpdate(v2 vDragStart, v2 vDragEnd)
{
	...
	m mrot = mid();
	mrot.x = g_WorldState.Camera.mview.x;
	mrot.y = g_WorldState.Camera.mview.y;
	mrot.z = g_WorldState.Camera.mview.z;
